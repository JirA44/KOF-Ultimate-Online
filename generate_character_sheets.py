# -*- coding: utf-8 -*-
"""
Générateur de fiches personnages KOF Ultimate Online
Parse tous les fichiers .cmd et génère des fiches Markdown
"""
...
def parse_command(cmd_string):
    ...
    result = []
    for part in parts:
        # Gère les combinaisons de boutons (a+b)
        if '+' in part:
            buttons = part.split('+')
            btn_names = []
            for btn in buttons:
                btn = btn.strip()
                # Retire les modificateurs spéciaux ($, ~, /, etc.)
                btn = re.sub(r'[~$/]', '', btn)
...
def extract_moves_from_cmd(cmd_file_path):
    ...
    try:
        ...
        for name, command in matches:
            ...
            # Catégorisation basée sur le nom et la commande
...
def generate_character_sheet(char_name, char_folder, moves):
    """Génère une fiche Markdown pour un personnage"""
...
"""
    ...
    # Super Moves
    if moves['supers']:
        md += "## 🔥 SUPER MOVES / DESPERATION MOVES\n\n"
...
def main():
    ...
    index_md = """# 📚 INDEX DES FICHES PERSONNAGES - KOF ULTIMATE ONLINE
...
"""
    ...
    for char_folder in sorted(glob.glob(os.path.join(chars_dir, '*'))):
        ...
        if not cmd_files:
            print(f"⚠️  Pas de .cmd trouvé pour {char_name}")
            continue

        # Utilise le premier fichier .cmd trouvé
        cmd_file = cmd_files[0]

        print(f"📋 Traitement de {char_name}...")

        # Extrait les mouvements
        moves = extract_moves_from_cmd(cmd_file)

        # Vérifie qu'il y a des mouvements
        ...
        print(f"   ✅ {total_moves} techniques trouvées")
        processed += 1

    # Sauvegarde l'index
    index_md += f"\n---\n\n**Total**: {processed} personnages documentés\n"

    with open(os.path.join(output_dir, 'INDEX.md'), 'w', encoding='utf-8') as f:
        f.write(index_md)

    print(f"\n✅ TERMINÉ! {processed} fiches générées dans {output_dir}")
    print(f"📖 Ouvre INDEX.md pour voir la liste complète")
