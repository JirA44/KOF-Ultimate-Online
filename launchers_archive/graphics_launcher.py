...
# -*- coding: utf-8 -*-
"""
KOF ULTIMATE - Lanceur Graphique Avancé
Applique des presets visuels et lance le jeu avec effets rétro-modernes
"""
...
class GraphicsLauncher:
    ...
    def load_presets(self):
        ...
        if not self.presets_file.exists():
            print(f"{Colors.RED}✗ Fichier presets manquant!{Colors.RESET}")
            return {}
    ...
    def backup_config(self):
        ...
        print(f"{Colors.GREEN}✓ Config sauvegardée: {backup_file.name}{Colors.RESET}")
    ...
    def apply_preset(self, preset_name):
        ...
        if preset_name not in self.presets['presets']:
            print(f"{Colors.RED}✗ Preset '{preset_name}' introuvable!{Colors.RESET}")
            return False

        preset = self.presets['presets'][preset_name]

        print(f"\n{Colors.CYAN}{Colors.BOLD}🎨 Application du preset: {preset['name']}{Colors.RESET}")
        ...
        # Modifier les paramètres config
        ...
        print(f"{Colors.GREEN}✓ Configuration mise à jour:{Colors.RESET}")
        print(f"  Résolution: {video_settings['Width']}x{video_settings['Height']}")
        ...
        print(f"  Plein écran: {'OUI' if video_settings['FullScreen'] else 'NON'}")

        # Afficher les effets
        if 'effects' in preset:
            effects = preset['effects']
            print(f"\n{Colors.MAGENTA}✨ Effets activés:{Colors.RESET}")
            if effects.get('crt_filter'):
                print(f"  • Filtre CRT (courbure: {effects.get('crt_curvature', 0.3)})")
            if effects.get('scanlines'):
                print(f"  • Scanlines (intensité: {effects.get('scanline_intensity', 0.2)})")
            if effects.get('bloom'):
                print(f"  • Bloom lumineux (intensité: {effects.get('bloom_intensity', 0.3)})")
            if effects.get('vignette'):
                print(f"  • Vignette (intensité: {effects.get('vignette', 0.3)})")
    ...
    def update_section(self, config, section_name, settings):
        ...
        for line in lines:
            ...
            # Modifier les paramètres dans la section
    ...
    def show_menu(self):
        ...
        print(f"{Colors.MAGENTA}{Colors.BOLD}{'🎮 KOF ULTIMATE - LANCEUR GRAPHIQUE':^80}{Colors.RESET}")
        ...
        for i, (key, preset) in enumerate(presets.items(), 1):
            is_current = " ← ACTUEL" if key == current else ""
            ...
            fullscreen = "Plein écran" if preset['video']['FullScreen'] else "Fenêtré"
            print(f"   {Colors.YELLOW}Résolution: {width}x{height} ({fullscreen}){Colors.RESET}\n")
    ...
    def launch_game(self):
        ...
        try:
            ...
            print(f"{Colors.GREEN}✓ Jeu lancé!{Colors.RESET}")
            return True
            ...
            try:
                ...
                if 1 <= choice_num <= num_presets:
                    ...
                    if self.apply_preset(preset_key):
                        print(f"\n{Colors.GREEN}✓ Preset appliqué avec succès!{Colors.RESET}")
