...
# -*- coding: utf-8 -*-
"""
KOF ULTIMATE - Test Visuel Automatisé
...
# Installer pyautogui si nécessaire
try:
    import pyautogui
    import win32gui
    import win32con
...
class VisualTester:
    ...
    def test_menu_visual(self):
        ...
        print(f"\n{Colors.CYAN}{Colors.BOLD}🎨 TEST VISUEL DU MENU{Colors.RESET}")
        ...
        if screenshot:
            ...
            if result['dark']:
                print(f"{Colors.RED}  ❌ Fond sombre détecté - animations possiblement absentes{Colors.RESET}")
            else:
                print(f"{Colors.GREEN}  ✓ Fond visuellement actif{Colors.RESET}")
        ...
        print(f"\n{Colors.GREEN}✓ Navigation terminée{Colors.RESET}")

    def test_eve_character(self):
        """Teste le personnage Eve corrigé"""
        print(f"\n{Colors.CYAN}{Colors.BOLD}🎭 TEST PERSONNAGE EVE{Colors.RESET}")
        print(f"{Colors.CYAN}{'='*80}{Colors.RESET}\n")
        ...
        # Valider
        print(f"{Colors.CYAN}Entrée dans VS MODE...{Colors.RESET}")
        ...
        if screenshot:
            ...
            if result['mostly_black']:
                print(f"{Colors.RED}  ❌ Écran noir - Eve possiblement en erreur{Colors.RESET}")
            else:
                print(f"{Colors.GREEN}  ✓ Eve charge correctement!{Colors.RESET}")
    ...
    def check_log(self):
        """Vérifie le log pour erreurs Eve"""
        ...
        if eve_errors:
            print(f"{Colors.RED}❌ Erreurs Eve détectées:{Colors.RESET}")
            for error in eve_errors[:5]:
                print(f"  - {error.strip()}")
            return False
        else:
            print(f"{Colors.GREEN}✓ Aucune erreur Eve dans le log!{Colors.RESET}")
            return True
    ...
    def show_results(self):
        ...
        print(f"{Colors.MAGENTA}{Colors.BOLD}{'RÉSULTATS DES TESTS':^80}{Colors.RESET}")
        print(f"{Colors.MAGENTA}{Colors.BOLD}{'='*80}{Colors.RESET}\n")

        for result in self.results:
            if 'error' in result:
                print(f"{Colors.RED}❌ {result['test']}: Erreur d'analyse{Colors.RESET}")
            else:
                status = "✓" if not result['dark'] and not result['mostly_black'] else "❌"
                ...
                print(f"  Luminosité: {result['brightness']:.1f}/255")
                print(f"  Screenshot: {result['screenshot']}\n")

        print(f"\n{Colors.CYAN}📁 Tous les screenshots: {self.screenshots_dir}{Colors.RESET}\n")

    def run_full_test(self):
        """Lance le test complet"""
        print(f"\n{Colors.MAGENTA}{Colors.BOLD}{'='*80}{Colors.RESET}")
        print(f"{Colors.MAGENTA}{Colors.BOLD}{'🎮 TEST VISUEL AUTOMATISÉ 🎮':^80}{Colors.RESET}")
        print(f"{Colors.MAGENTA}{Colors.BOLD}{'='*80}{Colors.RESET}\n")
        ...
        try:
            ...
            if eve_ok:
                print(f"{Colors.GREEN}🎉 PERSONNAGE EVE FONCTIONNE!{Colors.RESET}\n")
            else:
                print(f"{Colors.RED}❌ Personnage Eve a des erreurs{Colors.RESET}\n")
